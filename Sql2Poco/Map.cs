...
namespace Sql2Poco
{
    ...
    {
        public static string DBType2CSType(string p, out string SqlDbType, bool nullable=true)
        {
            switch (p.ToLower())
            {
                case "bigint":
        ...
        public static string FieldValueString(string csType, object value)
        {
            if (csType.Contains("DateTime")) return @"DateTime.Parse(""" + value + @""")";
            if (csType.Contains("string")) return @"""" + value + @"""";
